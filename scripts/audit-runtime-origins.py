...
directory, and bounded compiler-generated tail shapes observed in the target.
The report does not edit reconstruction ledgers or grant authored or
...
AUDIT_EVIDENCE_ID = "pinned-runtime-origin-audit-2026-09-19"


def find_msvc_root() -> Path:
    override = os.environ.get("TH09_MSVC71_ROOT")
    candidates = [
        Path(override) if override else None,
        ROOT / ".tools" / "msvc710",
        Path("/home/pentester/coding/codex_ida/th095-reconstruction/th095/.tools/msvc710"),
    ]
    for candidate in candidates:
        if candidate is not None and (candidate / "Vc7" / "bin" / "cl.exe").is_file():
            return candidate
    raise ValueError("missing pinned VC7.1 toolchain; set TH09_MSVC71_ROOT")


def library_paths() -> tuple[tuple[str, Path], ...]:
    root = find_msvc_root()
    return (
        ("d3dx8", root / "Vc7/PlatformSDK/Lib/d3dx8.lib"),
        ("libcmt", root / "Vc7/lib/libcmt.lib"),
        ("libcpmt", root / "Vc7/lib/libcpmt.lib"),
    )
RELOCATION_WIDTHS = {
...
class PEImage:
    def __init__(self, data: bytes):
        self.data = data
        if checked_slice(data, 0, 2, "DOS signature") != b"MZ":
            raise ValueError("missing DOS signature")
        pe = struct.unpack("<I", checked_slice(data, 0x3C, 4, "PE offset"))[0]
        if checked_slice(data, pe, 4, "PE signature") != b"PE\0\0":
            raise ValueError("missing PE signature")
        machine, count, _, _, _, optional_size, _ = struct.unpack(
            "<HHIIIHH", checked_slice(data, pe + 4, 20, "COFF header")
        )
        if machine != 0x14C or optional_size < 96:
            raise ValueError("expected i386 PE32 image")
        optional = checked_slice(data, pe + 24, optional_size, "PE optional header")
        ...
        for index in range(count):
            fields = struct.unpack(
                ...
                checked_slice(
                    data,
                    pe + 24 + optional_size + 40 * index,
                    40,
                    "PE section header",
                ),
            )
            virtual_size, rva, raw_size, raw_offset = fields[1:5]
            checked_slice(data, raw_offset, raw_size, "PE section data")
    ...
    def read(self, address: int, size: int) -> bytes:
        for section in self.sections:
            if section.start <= address and address + size <= section.end:
                return checked_slice(
                    self.data,
                    section.raw_offset + address - section.start,
                    size,
                    "mapped PE range",
                )
...
def audit(minimum_nonrelocation_bytes: int) -> dict[str, object]:
    ...
    for archive, path in library_paths():
        if not path.is_file():
    ...
    for row in rows:
        ...
        if origins == {"library"}:
            ...
            boundary_review = "ida_extent_matches_associative_coff_shape"
            ...
            boundary_review = "ida_extent_matches_pinned_library_shape"
