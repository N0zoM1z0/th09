// Shared type-8/type-9 update callback in the EclManager translation unit.
// TH09 retains the Float3::FromAngleMagnitude receiver in ECX across its
// call; visibility of that maintained definition reproduces this call site.

#include "AnmManager.hpp"
#include "AsciiManager.hpp"
#include "BulletManager.hpp"
#include "ExAttackController.hpp"
#include "ExAttackInterpolation.hpp"
#include "ExAttackType8GameManagerView.hpp"
#include "PlayerLifecycleView.hpp"

struct ExAttackType89UpdateExtra
{
    int state00;
    int unknown04;
    float angle08;
    float angle0C;
    float unknown10;
    Float3 motion14;
    Float3 point20;
    Float3 point2C;
    Float3 control38;
    Float3 control44;
    Float3 spawn50;
};
typedef char ExAttackType89UpdateExtraSizeIs5C[
    (sizeof(ExAttackType89UpdateExtra) == 0x5C) ? 1 : -1];

struct ExAttackType89UpdateRecord
{
    int unknown00;
    int opponentSide04;
    int side08;
    int active0C;
    ZunTimer timer10;
    void *dynamicData1C;
    Float3 position20;
    int vmCount2C;
    int unknown30;
    ExAttackType89UpdateExtra *extra34;
    int parameter38;
    float rotation3C;
};

struct ExAttackType89PlayerAnmView
{
    unsigned char unknown000[0xBC];
    AnmLoaded *anmFileBC;
};

struct ExAttackType89BulletDescriptorStorage
{
    unsigned char storage[0x214];
    ExAttackType89BulletDescriptorStorage();
};
typedef char ExAttackType89BulletDescriptorStorageSizeIs214[
    (sizeof(ExAttackType89BulletDescriptorStorage) == 0x214) ? 1 : -1];

float __stdcall AddNormalizeAngle(float angle, float delta);

int __fastcall ExAttackUpdateCallbackType8_9(ExAttackRecord *base)
{
    ExAttackType89UpdateRecord *record =
        reinterpret_cast<ExAttackType89UpdateRecord *>(base);
    ExAttackType89UpdateExtra *extra = record->extra34;
    Float3 collisionSize(24.0f, 24.0f, 0.0f);

    switch (extra->state00)
    {
    case 2:
        if (record->timer10 > 20)
            return 1;
        break;

    case 1:
        if (record->timer10 > 20)
        {
            if (record->timer10.HasTickedEvery(
                    10 - g_GameManager.difficulty11C))
            {
                float bulletAngle = AddNormalizeAngle(
                    extra->angle0C, 0.10471976f);
                extra->angle0C = bulletAngle;

                ExAttackType89BulletDescriptorStorage storage;
                BulletSpawnDescriptor *descriptor =
                    reinterpret_cast<BulletSpawnDescriptor *>(&storage);
                descriptor->position = record->position20;
                descriptor->bulletType = 6;
                descriptor->aimMode = 5;
                descriptor->count1 =
                    g_GameManager.sides[1 - record->opponentSide04]
                            .player04->header24.state7C /
                        5 +
                    2;
                descriptor->count2 = 1;
                descriptor->color = 2;
                descriptor->angle = bulletAngle;
                descriptor->speed1 = 0.0f;

                descriptor->InstallWaitTransform(0, 0, 80);
                descriptor->InstallVectorAccelerationTransform(
                    1, 0, 120,
                    (g_GameManager.sides[1 - record->opponentSide04]
                             .player04->header24.state7C *
                         0.1f +
                     2.0f) *
                        0.0083333338f,
                    -999.0f);

                descriptor->angleStep = 0.0f;
                descriptor->speed2 = 0.0f;
                descriptor->unknown1FA = 0;
                descriptor->transformFlags = 0x00020014;
                g_GameManager.sides[record->opponentSide04]
                    .etama08->SpawnBulletPatternPrimary(descriptor);
            }

            if (g_GameManager.sides[record->opponentSide04]
                    .player04->CheckBulletCollision(
                    reinterpret_cast<PlayerPositionView *>(
                        &record->position20),
                    reinterpret_cast<PlayerPositionView *>(
                        &collisionSize),
                    0) == 2)
            {
                extra->state00 = 2;
                record->timer10 = 0;
                reinterpret_cast<AnmVm *>(
                    record->dynamicData1C)->SetInterrupt(1);
                record->rotation3C = extra->angle08;
                return 0;
            }

            extra->motion14.FromAngleMagnitude(extra->angle08, 1.5f);
            record->position20 += extra->motion14;

            if (record->position20.y >= 480.0f ||
                record->position20.y <= -32.0f ||
                record->position20.x >= 176.0f ||
                record->position20.x <= -176.0f)
            {
                return 1;
            }
        }

        record->rotation3C = extra->angle08;
        break;

    case 0:
        ExAttackInterpolate2D(
            &record->position20.x,
            &extra->point2C.x,
            &extra->point20.x,
            &extra->control44.x,
            &extra->control38.x,
            record->timer10,
            90.0f);

        if (record->timer10 > 90)
        {
            reinterpret_cast<ExAttackType89PlayerAnmView *>(
                g_GameManager.sides[record->side08].player04)
                ->anmFileBC->ExecuteAnmIdx(
                    reinterpret_cast<AnmVm *>(record->dynamicData1C), 9);
            ++extra->state00;
            record->position20 = extra->spawn50;
            record->unknown00 = record->opponentSide04;
            record->timer10 = 0;
        }
        break;
    }

    return 0;
}
