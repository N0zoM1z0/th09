...
typedef char ExAttackType14ExtraSizeIsF48[
    (sizeof(ExAttackType14Extra) == 0xF48) ? 1 : -1];
...
int __fastcall ExAttackInitializeCallbackType14(ExAttackRecord *base)
{
    ...
    reinterpret_cast<ExAttackAllocateRecordView *>(record)->
        AllocateDynamicData(1, sizeof(ExAttackType14Extra));
    ...
    ExAttackType14Extra *extra = record->extra34;

    float angle =
        g_ReplayRng.GetRandomF32InRange(0.19634955f) + 1.5707964f;
    ...
    extra->spawnPosition40.y =
        g_ReplayRng.GetRandomF32InRange(128.0f);
