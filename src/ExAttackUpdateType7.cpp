...
typedef char ExAttackType7UpdateExtraSizeIs5C[
    (sizeof(ExAttackType7UpdateExtra) == 0x5C) ? 1 : -1];

struct ExAttackType7UpdateRecord
{
    int unknown00;
    int opponentSide04;
    int side08;
    ...
    int vmCount2C;
    int unknown30;
    ExAttackType7UpdateExtra *extra34;
    int parameter38;
    float rotation3C;
};

struct ExAttackType7InterruptView
{
    void SetInterrupt(short interrupt);
};

struct ExAttackType7PlayerAnmView
{
...
struct ExAttackType7BulletDescriptorStorage
{
    ...
    ExAttackType7BulletDescriptorStorage();
};
...
{
    ExAttackType7UpdateRecord *record =
        reinterpret_cast<ExAttackType7UpdateRecord *>(base);
    ExAttackType7UpdateExtra *extra = record->extra34;
    ...
    case 1:
        if (record->timer10 > 20)
        {
                    ...
                    10 - g_GameManager.difficulty11C))
            {
                ...
                descriptor->bulletType = 0;
                descriptor->aimMode = 3;
                descriptor->angle = AddNormalizeAngle(
                    3.1415927f, -extra->angle0C);
    ...
    case 0:
        ...
        {
            reinterpret_cast<ExAttackType7PlayerAnmView *>(
