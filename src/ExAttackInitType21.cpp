...
typedef char ExAttackType21VertexSizeIs1C[
    (sizeof(ExAttackType21Vertex) == 0x1C) ? 1 : -1];

struct ExAttackType21Extra
{
    int state00;
    Float3 motion04;
    Float3 point10;
...
typedef char ExAttackType21VerticesAt4C[
    (offsetof(ExAttackType21Extra, vertices4C) == 0x4C) ? 1 : -1];
typedef char ExAttackType21HistoryAt3E8[
    (offsetof(ExAttackType21Extra, history3E8) == 0x3E8) ? 1 : -1];
typedef char ExAttackType21MagnitudesAt578[
    (offsetof(ExAttackType21Extra, magnitudes578) == 0x578) ? 1 : -1];
typedef char ExAttackType21StepsAt5FC[
    (offsetof(ExAttackType21Extra, angularSteps5FC) == 0x5FC) ? 1 : -1];
typedef char ExAttackType21PendingAt684[
    (offsetof(ExAttackType21Extra, drawPending684) == 0x684) ? 1 : -1];
typedef char ExAttackType21AngleAt688[
    (offsetof(ExAttackType21Extra, angle688) == 0x688) ? 1 : -1];

struct ExAttackType21RecordView
{
    int state00;
    int opponentSide04;
    int side08;
    ...
    int vmCount2C;
    int unknown30;
    ExAttackType21Extra *extra34;
};

struct ExAttackType21AnmOwner
{
    unsigned char unknown000[0x2D4];
    AnmLoaded *anm2D4;
};

struct ExAttackType21SideView
{
    unsigned char unknown00[0x0C];
    ExAttackType21AnmOwner *anmOwner0C;
    unsigned char unknown10[0x28];
};

struct ExAttackType21SideLookupView
{
    int state00;
    int opponentSide04;
    int side08;
    ExAttackType21SideView *GetSideState();
};

struct ExAttackType21AllocateView
{
...
int __fastcall ExAttackInitializeCallbackType21(ExAttackRecord *base)
{
    ExAttackType21RecordView *record =
        reinterpret_cast<ExAttackType21RecordView *>(base);

    reinterpret_cast<ExAttackType21AllocateView *>(record)
        ->AllocateDynamicData(1, sizeof(ExAttackType21Extra));

    reinterpret_cast<ExAttackType21SideLookupView *>(record)
        ->GetSideState()->anmOwner0C->anm2D4->ExecuteAnmIdx(
            reinterpret_cast<AnmVm *>(record->dynamicData1C), 37);

    ExAttackType21Extra *extra = record->extra34;
    ...
    extra->spawn40.y =
        g_ReplayRng.GetRandomF32InRange(128.0f) + 128.0f;
    ...
    extra->angle688 = -3.1415927f;

    ExAttackType21Vertex *vertex = &extra->vertices4C[0];
    vertex->rhw = 1.0f;
    vertex->u = 0.5f;
    vertex->v = 0.5f;
    ...
    do
    {
        ...
        *history += *spawn;
        ...
        uv += sizeof(ExAttackType21Vertex) / sizeof(float);
        ++history;
        ++magnitude;
        angle += 0.2026834f;
    }
