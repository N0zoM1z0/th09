...
| TOOLCHAIN-003 | compiler-observed | The two GameErrorContext units require optimized VC7.1 code shape: the tested `/Od /Ob1` profile produces 0x23/0x85-byte Reset/Flush bodies, `/O1` produces 0x0A/0x4E, and tested `/O2`/`/Ox` variants produce the target 0x0A/0x50 extents and exact bytes. This does not identify the original global optimization flags. | Focused compiler matrix using pinned VC7.1 `13.10.3077`; canonical units currently use one exact-producing `/O2` profile |
