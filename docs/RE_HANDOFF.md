...
`config/implemented.csv` contains 834 source-present authored mappings. Of
these, 688 are canonical exact and 146 retain honest non-exact compiler results.
`config/matches.csv` contains 688 complete target-bound VC7.1 matches totaling
89,168 exact authored bytes. Source presence, origin, exactness, product
...
**145 authored functions / 75,086 bytes without maintained source**, plus 146
