...
| Source-present authored mappings | 814 |
...
`config/implemented.csv` contains 814 source-present authored mappings. Of
these, 669 are canonical exact and 145 retain honest non-exact compiler results.
`config/matches.csv` contains 669 complete target-bound VC7.1 matches totaling
87,277 exact authored bytes. Source presence, origin, exactness, product
...
**166 authored functions / 77,792 bytes without maintained source**, plus 145
