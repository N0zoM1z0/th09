...
| Source-present authored mappings | 817 |
...
`config/implemented.csv` contains 817 source-present authored mappings. Of
these, 672 are canonical exact and 145 retain honest non-exact compiler results.
`config/matches.csv` contains 672 complete target-bound VC7.1 matches totaling
87,594 exact authored bytes. Source presence, origin, exactness, product
...
**163 authored functions / 77,475 bytes without maintained source**, plus 145
