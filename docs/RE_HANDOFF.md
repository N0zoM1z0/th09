...
| Source-present authored mappings | 770 |
| Canonical exact functions | 627 |
...
`config/implemented.csv` contains 770 source-present authored mappings. Of
these, 627 are canonical exact and 143 retain honest non-exact compiler results.
`config/matches.csv` contains 627 complete target-bound VC7.1 matches totaling
84,309 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 220
   authored functions / 81,260 bytes without maintained source. The routed
