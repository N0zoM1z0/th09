...
| Source-present authored mappings | 783 |
...
`config/implemented.csv` contains 783 source-present authored mappings. Of
these, 639 are canonical exact and 144 retain honest non-exact compiler results.
`config/matches.csv` contains 639 complete target-bound VC7.1 matches totaling
85,021 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 207
   authored functions / 80,467 bytes without maintained source. The routed
