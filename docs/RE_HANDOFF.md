...
`config/implemented.csv` contains 741 source-present authored mappings. Of
these, 604 are canonical exact and 137 retain honest non-exact compiler results.
`config/matches.csv` contains 604 complete target-bound VC7.1 matches totaling
82,575 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 249
   authored functions / 85,451 bytes without maintained source. The routed
   ...
   Prefer adjacent small ECL/Ascii/Player helpers before another large dispatcher.
