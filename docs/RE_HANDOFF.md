...
| Source-present authored mappings | 784 |
| Canonical exact functions | 640 |
...
`config/implemented.csv` contains 784 source-present authored mappings. Of
these, 640 are canonical exact and 144 retain honest non-exact compiler results.
`config/matches.csv` contains 640 complete target-bound VC7.1 matches totaling
85,100 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 206
   authored functions / 80,388 bytes without maintained source. The routed
