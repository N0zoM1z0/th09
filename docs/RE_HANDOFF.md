...
| Source-present authored mappings | 673 |
| Canonical exact functions | 541 |
...
`config/implemented.csv` contains 673 source-present authored mappings. Of
these, 541 are canonical exact and 132 retain honest non-exact compiler results.
`config/matches.csv` contains 541 complete target-bound VC7.1 matches totaling
77,936 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 317
   authored functions / 91,314 bytes without maintained source. The routed
