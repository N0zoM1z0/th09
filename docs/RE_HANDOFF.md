...
| Source-present authored mappings | 842 |
...
`config/implemented.csv` contains 842 source-present authored mappings. Of
these, 696 are canonical exact and 146 retain honest non-exact compiler results.
`config/matches.csv` contains 696 complete target-bound VC7.1 matches totaling
89,936 exact authored bytes. Source presence, origin, exactness, product
...
**137 authored functions / 74,318 bytes without maintained source**, plus 146
