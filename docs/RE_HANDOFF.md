...
| Source-present authored mappings | 769 |
| Canonical exact functions | 626 |
...
`config/implemented.csv` contains 769 source-present authored mappings. Of
these, 626 are canonical exact and 143 retain honest non-exact compiler results.
`config/matches.csv` contains 626 complete target-bound VC7.1 matches totaling
84,277 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 221
   authored functions / 81,292 bytes without maintained source. The routed
