...
| Source-present authored mappings | 727 |
...
`config/implemented.csv` contains 727 source-present authored mappings. Of
these, 591 are canonical exact and 136 retain honest non-exact compiler results.
`config/matches.csv` contains 591 complete target-bound VC7.1 matches totaling
81,372 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 263
   authored functions / 87,046 bytes without maintained source. The routed
