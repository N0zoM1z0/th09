...
| Source-present authored mappings | 698 |
| Canonical exact functions | 565 |
...
`config/implemented.csv` contains 698 source-present authored mappings. Of
these, 565 are canonical exact and 133 retain honest non-exact compiler results.
`config/matches.csv` contains 565 complete target-bound VC7.1 matches totaling
78,818 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 292
   authored functions / 90,123 bytes without maintained source. The routed
