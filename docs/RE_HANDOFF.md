...
| Source-present authored mappings | 900 |
...
`config/implemented.csv` contains 900 source-present authored mappings. Of
these, 741 are canonical exact and 159 retain honest non-exact compiler results.
`config/matches.csv` contains 741 complete target-bound VC7.1 matches totaling
101,482 exact authored bytes. Source presence, origin, exactness, product
...
**79 authored functions / 58,839 bytes without maintained source**, plus 159
