...
| Source-present authored mappings | 759 |
| Canonical exact functions | 620 |
...
`config/implemented.csv` contains 759 source-present authored mappings. Of
these, 620 are canonical exact and 139 retain honest non-exact compiler results.
`config/matches.csv` contains 620 complete target-bound VC7.1 matches totaling
83,522 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 231
   authored functions / 83,614 bytes without maintained source. The routed
