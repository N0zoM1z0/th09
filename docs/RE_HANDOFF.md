...
| Source-present authored mappings | 742 |
| Canonical exact functions | 605 |
...
`config/implemented.csv` contains 742 source-present authored mappings. Of
these, 605 are canonical exact and 137 retain honest non-exact compiler results.
`config/matches.csv` contains 605 complete target-bound VC7.1 matches totaling
82,603 exact authored bytes. Source presence, origin, exactness, product
...
2. **Recover authored source in bounded subsystem seams.** There are 248
   authored functions / 85,423 bytes without maintained source. The routed
   ...
   separate durable negative. Prefer adjacent small ECL/Ascii/Player helpers
   before another large dispatcher.
