...
| Source-present authored mappings | 960 |
| Canonical exact functions | 771 |
...
`config/implemented.csv` contains 960 source-present authored mappings. Of
these, 771 are canonical exact and 189 retain honest non-exact compiler results.
`config/matches.csv` contains 771 complete target-bound VC7.1 matches totaling
117,032 exact authored bytes. Source presence, origin, exactness, product
...
**19 authored functions / 21,457 bytes without maintained source**, plus 189
